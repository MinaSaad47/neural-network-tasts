...
class Task2(Task):
    ...
    def visualize_dataset(self):
        ...
        mse_threshold = tk.DoubleVar(value=0.02)
        ...
        def train_cb():
            ...
            mse_th = mse_threshold.get()
            df = self.dataset.copy()

            # return adaline model
            accuracy, mse = models.task2.train(
                df, f1, f2, c1, c2, eta, nb_epochs, is_bias, mse_th)
        ...
        tk.Label(traning_frame, text='mse threshold').grid(row=4, column=0)
        entry = tk.Entry(traning_frame, textvariable=mse_threshold)
